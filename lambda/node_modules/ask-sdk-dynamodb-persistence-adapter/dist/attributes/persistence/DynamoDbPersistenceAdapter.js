...
class DynamoDbPersistenceAdapter {
        ...
        if (this.createTable) {
            ...
            this.dynamoDBClient.createTable(createTableParams, (createTableErr) => {
                if (createTableErr && createTableErr.code !== 'ResourceInUseException') {
                    throw (0, ask_sdk_core_1.createAskSdkError)(this.constructor.name, `Could not create table (${this.tableName}): ${createTableErr.message}`);
                }
            });
        }
    }
    ...
    async getAttributes(requestEnvelope) {
        ...
        try {
            ...
            throw (0, ask_sdk_core_1.createAskSdkError)(this.constructor.name, `Could not read item (${attributesId}) from table (${getParams.TableName}): ${err.message}`);
        }
    ...
    async saveAttributes(requestEnvelope, attributes) {
        ...
        try {
            ...
            throw (0, ask_sdk_core_1.createAskSdkError)(this.constructor.name, `Could not save item (${attributesId}) to table (${putParams.TableName}): ${err.message}`);
        }
    }
    /**
    ...
    async deleteAttributes(requestEnvelope) {
        ...
        try {
            ...
            throw (0, ask_sdk_core_1.createAskSdkError)(this.constructor.name, `Could not delete item (${attributesId}) from table (${deleteParams.TableName}): ${err.message}`);
        }
    }
}
